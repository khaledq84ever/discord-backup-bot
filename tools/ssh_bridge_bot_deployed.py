...
REQUIRED_VARS = ["DISCORD_TOKEN", "OWNER_ID", "SSH_HOST", "SSH_USER"]
missing = [v for v in REQUIRED_VARS if not os.environ.get(v)]
if missing:
    raise SystemExit(f"Missing required env vars: {', '.join(missing)}")
if not os.environ.get("SSH_PASSWORD") and not os.environ.get("SSH_PRIVATE_KEY"):
    raise SystemExit("Set either SSH_PASSWORD or SSH_PRIVATE_KEY")
...
def _connect_shell():
    ...
    print("Connecting SSH shell...", flush=True)
    ...
    _ssh_client = ssh
    _shell = shell
    print("SSH shell connected.", flush=True)
...
def run_in_shell_streaming(command: str, loop, on_update):
    ...
    while time.time() < deadline:
        try:
            ...
            print(f"run_in_shell_streaming error: {e!r}", flush=True)
            _discard_shell()
            return f"(shell error: {e})"
    else:
        print(f"Timed out. Partial buffer: {buf[:500]!r}", flush=True)
        _discard_shell()
...
async def run_command(interaction: discord.Interaction, command: str):
    if interaction.user.id != OWNER_ID:
        print(f"Unauthorized /run attempt from user.id={interaction.user.id} ({interaction.user})", flush=True)
